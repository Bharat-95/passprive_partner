...
function Amount({ value, className = "", valueClassName = "", currencyClassName = "" }) {
  ...
  return (
    <span className={className}>
      <span className={currencyClassName || "text-[10px] font-semibold uppercase tracking-wide text-gray-500"}>MUR</span>{" "}
...
function StatusPill({ status }) {
  ...
  const cls =
    s === "paid" || s === "approved" || s === "verified"
      ...
      : s === "failed" || s === "rejected" || s === "on_hold" || s === "on hold" || s === "not_started"
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-3">
              ...
              <div className="mt-1 text-sm font-semibold text-gray-900">{restaurantName || "-"}</div>
            </div>
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-3">
...
async function createPayoutRequest(userId, restaurantId, amount, method, details) {
...
export default function RestaurantPartnerPayoutsPage() {
  ...
  const [paymentDetailsByRestaurant, setPaymentDetailsByRestaurant] = useState({});
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setRestaurants(myRestaurants);

        if (!restaurantIds.length) {
          if (!cancelled) {
            setOrders([]);
            setPaymentDetailsByRestaurant({});
            setPayoutRequests([]);
          }
          return;
        }

        const [ordersData, paymentRes, payoutReqRes] = await Promise.all([
          loadRestaurantOrders(restaurantIds),
          supabaseBrowser
            .from("restaurant_payment_details")
            .select("restaurant_id,payout_method,settlement_cycle,commission_percent,currency,kyc_status")
        ...
        if (!cancelled) {
          ...
          (paymentRes.data || []).forEach((r) => {
            map[String(r.restaurant_id)] = r;
          });
          setPaymentDetailsByRestaurant(map);
  ...
  const settlementByRestaurant = useMemo(() => {
    ...
    orders.filter(isValidOrderForSettlement).forEach((o) => {
      ...
      const commissionPct = Number(paymentDetailsByRestaurant[rid]?.commission_percent || 0);
      ...
      map[rid].businessMade += total;
      map[rid].commissionDue += commission;
      map[rid].validOrderCount += 1;

      if (isCashMethod(o.payment_method)) map[rid].cashReceived += total;
      else map[rid].onlineCollected += total;
    });
    ...
    return map;
  }, [orders, paymentDetailsByRestaurant]);
  ...
  const settlement = useMemo(() => {
    ...
    Object.values(settlementByRestaurant).forEach((s) => {
    ...
    return base;
  }, [settlementByRestaurant]);

  const eligibleRestaurants = useMemo(() => {
    return restaurants
      .map((r) => {
        const rid = String(r.id);
        const pay = settlementByRestaurant[rid] || {
          partnerPayable: 0,
          businessMade: 0,
          commissionDue: 0,
          toPassPrive: 0,
        ...
        return {
          ...r,
          ...pay,
          payment: paymentDetailsByRestaurant[rid] || null,
        };
      })
      .sort((a, b) => Number(b.partnerPayable || 0) - Number(a.partnerPayable || 0));
  }, [restaurants, settlementByRestaurant, paymentDetailsByRestaurant]);
  ...
  useEffect(() => {
    ...
    const exists = eligibleRestaurants.some((r) => String(r.id) === String(selectedRestaurantId));
    if (!exists || selectedRestaurantId === "ALL") {
      const preferred = eligibleRestaurants.find((r) => r.partnerPayable > 0) || eligibleRestaurants[0];
      setSelectedRestaurantId(String(preferred.id));
    }
  }, [eligibleRestaurants, selectedRestaurantId]);

  const selectedRestaurant = useMemo(() => {
    return eligibleRestaurants.find((r) => String(r.id) === String(selectedRestaurantId)) || null;
  }, [eligibleRestaurants, selectedRestaurantId]);

  const selectedPayoutAmount = Number(selectedRestaurant?.partnerPayable || 0);
  const selectedKyc = selectedRestaurant?.payment?.kyc_status || "NOT_STARTED";
  const selectedMethod = selectedRestaurant?.payment?.payout_method || "BANK_TRANSFER";
  const selectedCycle = selectedRestaurant?.payment?.settlement_cycle || "-";

  const recentPayouts = useMemo(() => {
    const restaurantNameById = {};
    restaurants.forEach((r) => {
      restaurantNameById[String(r.id)] = r.name;
    });

    return payoutRequests
      .map((r) => ({
        id: r.id,
        restaurantName: restaurantNameById[String(r.restaurant_id)] || "Restaurant",
        method: r.method || "BANK_TRANSFER",
  ...
  const filteredRecentPayouts = useMemo(() => {
    ...
    return recentPayouts.filter((p) => {
      ...
      if (!q) return true;
      const hay = `${p.id} ${p.restaurantName} ${p.method} ${p.status} ${p.reference}`.toLowerCase();
  ...
  const requestDisabledReason = useMemo(() => {
    if (!selectedRestaurant) return "Select a restaurant";
    if (!selectedRestaurant.payment) return "Restaurant payment details missing";
    if (String(selectedKyc).toUpperCase() !== "VERIFIED") return "KYC must be VERIFIED";
    if (selectedPayoutAmount <= 0) return "No payable amount";
    return "";
  }, [selectedRestaurant, selectedKyc, selectedPayoutAmount]);
  ...
  const handleRequestPayout = async () => {
    ...
    try {
      ...
      setRequestSuccess(
        `Payout request created for ${selectedRestaurant.name}: MUR ${formatMoney(selectedPayoutAmount)} via ${selectedMethod}.`
      );
    } catch (e) {
  ...
  const handleExport = () => {
    ...
    rows.push(["Restaurant", "Business Made", "Online Collected", "Commission Due", "Payable To Partner", "To PassPrive", "Valid Orders"]);
    eligibleRestaurants.forEach((r) => {
      rows.push([
        r.name,
        Number(r.businessMade || 0),
        Number(r.onlineCollected || 0),
        Number(r.commissionDue || 0),
        Number(r.partnerPayable || 0),
        Number(r.toPassPrive || 0),
        Number(r.validOrderCount || 0),
      ]);
    });

    rows.push([]);
    rows.push(["Request ID", "Restaurant", "Requested Amount", "Method", "Status", "Requested At", "Processed At", "Reference"]);
    ...
    a.href = url;
    a.download = `restaurant-payout-settlement-${date}.csv`;
  ...
  return (
    <div className="min-h-screen" style={{ fontFamily: '"Space Grotesk", "Sora", sans-serif' }}>
      <div className="mx-auto max-w-6xl px-6 py-4 space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {requesting ? "Sending..." : `Request ${selectedPayoutAmount > 0 ? `MUR ${formatMoney(selectedPayoutAmount)}` : "Payout"}`}
            </button>
          </div>
        </div>
        ...
        {requestSuccess ? (
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700">{requestSuccess}</div>
        ) : null}
                ...
                <tbody>
                  {filteredRecentPayouts.map((p) => (
                    <tr key={p.id} className="border-t border-gray-100 hover:bg-gray-50/60 transition-colors">
                      ...
                      <td className="py-3 pr-4 text-gray-700">{p.restaurantName}</td>
                      <td className="py-3 pr-4">
                        <Amount
                          value={p.amount}
                          className="inline-flex items-baseline gap-1"
      ...
      <RequestPayoutModal
        ...
        kyc={selectedKyc}
        cycle={selectedCycle}
        onlineCollected={selectedRestaurant?.onlineCollected || 0}
        commissionDue={selectedRestaurant?.commissionDue || 0}
        toPassPrive={selectedRestaurant?.toPassPrive || 0}
      />
    </div>
  );
}
