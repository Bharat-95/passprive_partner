...
export default function PartnerCataloguePage() {
    ...
    try {
      const [categoryRes, itemRes, serviceMenuRes] = await Promise.all([
        ...
        supabaseBrowser
          ...
          .select(
            "id,store_id,category_id,title,description,price,sku,image_url,service_for,is_available,item_type,is_billable,duration_minutes,supports_slot_booking,track_inventory,stock_qty,low_stock_threshold,stock_status,allow_backorder,is_image_catalogue,sort_order,created_at,updated_at"
          )
          .eq("store_id", storeId)
          .order("sort_order", { ascending: true })
  ...
  useEffect(() => {
    setExpandedServiceCategoryIds((prev) => {
      ...
      return sortedCategories.map((category) => String(category.id));
    });
  }, [sortedCategories]);
  ...
  const startAddMoreForCategory = (category) => {
    setEditingItemId("");
    setItemImageFiles([]);
    setItemImagePreviewUrls([]);
    setItemForm((prev) => ({
    ...
    <div className="min-h-screen" style={{ fontFamily: '"Space Grotesk", "Sora", sans-serif' }}>
      <div className="mx-auto max-w-7xl space-y-6 px-6 py-4">
        ...
        {loadingStores || !selectedStore ? null : isServiceStore ? (
          <StoreServicesManager storeId={selectedStoreId} storeType={selectedStoreType} />
        ) : (
          <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
                          ...
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge tone={categoryItems.length ? "green" : "gray"}>
                                {categoryItems.length} {isServiceStore ? "service" : "item"}
                            ...
                            {isServiceStore ? (
                              <>
                                ...
                                <button
                                  ...
                                  className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-gray-200 bg-white p-0 text-gray-700 hover:bg-gray-50"
                              ...
                              <>
                                <button
                                  type="button"
                                  onClick={() => startAddMoreForCategory(category)}
                                  className="inline-flex h-9 items-center gap-2 rounded-full border border-gray-200 bg-white px-3 text-sm font-medium text-gray-700 hover:bg-gray-50"
                                >
                                  <Plus className="h-4 w-4" />
                                  Add More
            ...
            <div ref={itemFormRef} className="xl:col-span-2">
              <Card
                title="Item Form"
                subtitle="Add and manage catalogue items."
                right={
                  editingItemId ? (
                    <button
                      type="button"
        ...
        {loadingStores || !selectedStore || !isServiceStore ? null : (
          <Card
            title={
              <div className="flex items-center gap-2">
